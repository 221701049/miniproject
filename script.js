document.addEventListener('DOMContentLoaded', function () {
    ...
    function displayResults(data) {
        const filteredData = filterData(data); // Filter the data based on user input
    ...
    document.getElementById('vegetable-btn').addEventListener('click', () => startSpeechRecognition(document.getElementById('vegetable-select')));
    document.getElementById('district-btn').addEventListener('click', () => startSpeechRecognition(document.getElementById('district-select')));
    document.getElementById('market-btn').addEventListener('click', () => startSpeechRecognition(document.getElementById('market-select')));
    document.getElementById('date-btn').addEventListener('click', () => startSpeechRecognition(document.getElementById('date-input')));
    ...
    function handleDateVoiceInput() {
        if ('webkitSpeechRecognition' in window) {
            ...
            speak("தேதியை சொல்லுங்கள். உதாரணமாக, 'இன்று' அல்லது '2023-10-15'.");
    ...
    function parseDateInput(input) {
        ...
        } else if (input.includes("நாளை")) {
            const tomorrow = new Date(today);
            tomorrow.setDate(today.getDate() + 1);
            return `${tomorrow.getFullYear()}-${String(tomorrow.getMonth() + 1).padStart(2, '0')}-${String(tomorrow.getDate()).padStart(2, '0')}`; // Tomorrow's date
